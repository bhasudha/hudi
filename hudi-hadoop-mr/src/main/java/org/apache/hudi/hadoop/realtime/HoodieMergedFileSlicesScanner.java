 ...
 * file.
 *
 * //TODO SUDHA change map to external spillable map
 */
public class HoodieMergedFileSlicesScanner {
  ...
  public HoodieMergedFileSlicesScanner(HoodieMORIncrementalFileSplit split, JobConf jobConf, Reporter reporter) {
    this.split = split;
  ...
  public void scan(HoodieMORIncrementalRecordReader recordReader) throws IOException {
    for (FileSlice fileSlice: fileSlices) {
      Map<String, ArrayWritable> newRecords = scanFileSlice(fileSlice, recordReader);
  ...
  private Map<String, ArrayWritable> scanFileSlice(FileSlice fileSlice, HoodieMORIncrementalRecordReader recordReader) throws IOException {
    Map<String, ArrayWritable> baseFileRecords = fetchBaseFileRecordsMapForSlice(fileSlice);
    Map<String, HoodieRecord<? extends HoodieRecordPayload>> deltaRecords = fetchDeltaRecordMapForSlice(fileSlice, recordReader);
    if ((baseFileRecords == null || baseFileRecords.isEmpty()) && deltaRecords != null) {
      ...
      for (Entry<String, HoodieRecord<? extends HoodieRecordPayload>> entry: deltaRecords.entrySet()) {
        Option<ArrayWritable> aWritable = hoodieRecordToArrayWritable(entry.getValue(), recordReader);
    ...
    return applyUpdatesToBaseFile(baseFileRecords, deltaRecords, recordReader);
  }

  private Map<String, ArrayWritable> applyUpdatesToBaseFile(
      Map<String, ArrayWritable> baseFileRecords,
      Map<String, HoodieRecord<? extends HoodieRecordPayload>> deltaRecords,
      HoodieMORIncrementalRecordReader recordReader) throws IOException {
    ...
    while (iterator.hasNext()) {
      ...
      if (deltaRecords.containsKey(entry.getKey())) {
        Option<ArrayWritable> aWritable = hoodieRecordToArrayWritable(deltaRecords.get(entry.getKey()), recordReader);
        if (!aWritable.isPresent()) {
          ...
          try {
            ...
            entry.getValue().set(originalValue); //TODO SUDHA Does this replaces the original value in baseFileRecords map ?
  ...
  private Option<ArrayWritable> hoodieRecordToArrayWritable(
      HoodieRecord<? extends HoodieRecordPayload> record, HoodieMORIncrementalRecordReader recordReader) throws IOException {
  ...
  private Map<String, HoodieRecord<? extends HoodieRecordPayload>> fetchDeltaRecordMapForSlice(FileSlice fileSlice, HoodieMORIncrementalRecordReader recordReader) {
    ...
    return new HoodieMergedLogRecordScanner(
        FSUtils.getFs(split.getBasePath(), jobConf),split.getBasePath(),
        logFilesPath,
        ...
        jobConf.get(SPILLABLE_MAP_BASE_PATH_PROP, DEFAULT_SPILLABLE_MAP_BASE_PATH)).getRecords();
  }
